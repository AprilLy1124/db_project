...
$sql = "SELECT Email, Username, IFNULL(COUNT(ID),0) AS Propertynum from USER LEFT JOIN PROPERTY".
" ON Username=User_Name WHERE User_type=1 GROUP BY Username ORDER BY $column1 $order;";
...
if ($action=='search') {
  ...
  if ( $keyword!=""){
    ...
    $newsql = "SELECT Email, Username, IFNULL(COUNT(ID),0) AS Propertynum from USER LEFT JOIN PROPERTY".
  " ON Username=User_Name WHERE User_type=1 AND $column2 LIKE '%$keyword%' GROUP BY Username ORDER BY $column1 $order;";
  }
}
...
if ($action == "del_own") {
  if (isset($_POST['choose_owner'])) {
    foreach($_POST['choose_owner'] as $selected) {
        ...
        <form class="example" method="post">
          ...
          <div class="row">
            ...
            <div class="single-defination">
                <button class="genric-btn primary radius" type="submit" name="action" value="del_own"> Delete Owner Account </button>
            </div>
            </div>
          ...
          <div class="single-defination">
              <div>
                <select class="genric-btn default height radius" name = "tskoption">
                  <option selected="selected" value="Username">Owner</option>
            ...
            <div class="single-defination">
                <select class="genric-btn default height radius" name = "sort_type">
                  <option selected="selected" value="Username">Owner</option>
                  <option  value="Email">Email</option>
                  <option  value="Propertynum">Number of Property</option>
                </select>
          ...
          <div class="col-md-1">
          <div class="single-defination">
              <a href="13_view_owner.php" class="genric-btn primary radius" float='left'>↻</a>
          </div>
          </div>
          <br><br><br><br>
          </div>
...
<h3 class="mb-30 text-center">All Owners in System</h3>
...
<div class="progress-table-wrap">
  <div class="progress-table">
    <div class="table-head">
      ...
      <div class="percentage">Number of Property</div>
    </div>
      ...
      <div class="table-row">
      <tr>
        <div class="percentage"> <input type="checkbox" name="choose_owner[]" value="<?php echo $name ?>"><?php echo $name ?></div>
        <div class="percentage"><?php echo $email ?></div>
        <div class="percentage"><?php echo $pnum ?></div>
      </tr>
      </div>
