...
$sql = "SELECT Name, ID, Size_Acres,St_Address,City,ZIP,IsComm, IsPublic, Property_Type,  ".
"IFNULL(Admin_Name, 0), IFNULL(COUNT(Rating),0) AS Visits, AVG(Rating) AS Avg_Rating ".
"FROM PROPERTY LEFT JOIN VISIT ON ID=P_ID WHERE not (User_Name = '$username') AND ".
"Admin_Name IS NOT NULL GROUP BY ID ORDER BY $column1 $order;";
...
if ($action == 'search') {
  ...
  if ( $keyword!=""){
    ...
    " IFNULL(Admin_Name, 0), IFNULL(COUNT(Rating),0) AS Visits, AVG(Rating) AS Avg_Rating".
    " FROM PROPERTY LEFT JOIN VISIT ON ID=P_ID WHERE not (User_Name = '$username') "."
     AND Admin_Name IS NOT NULL AND $column2 LIKE '%$keyword%' GROUP BY ID ORDER BY $column1 $order;";
  }
  }
...
if ($action == 'detail') {
  if (isset($_POST['p_detail'])) {
    ...
    header('location: /5_1_own_det.php');
  } else {
...
if ($action == 'search2') {
  if ($_POST['minsearch']=="" && $_POST['maxsearch']=="") {
    ...
    "IFNULL(AVG(Rating),0) AS Avg_Rating FROM PROPERTY LEFT JOIN VISIT ON ID=P_ID WHERE not (User_Name = '$username') AND Admin_Name IS NOT NULL GROUP BY ID HAVING ";
    if ($_POST['minsearch']!="") {
      ...
      <div class="section-top-border">
          <form class="example" method="post">
            ...
            <div class="single-defination">
                ...
                <button class="genric-btn primary radius" type="submit" name="action" value="detail">View Property</button>
          ...
          <div class="col-md-1">
            <div class="single-defination">
              <div>
                <a href="5_view_other.php" class="genric-btn primary radius" float='left'>↻</a>
              </div>
          </div>
        </div>
        <br><br><br><br>
...
<h3 class="mb-30 text-center">All Other Valid Properties</h3>
