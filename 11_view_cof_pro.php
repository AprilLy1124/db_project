...
$sql = "SELECT Name, ID, Size_Acres, St_Address, City, ZIP, IsPublic, IsComm,Property_Type, Admin_Name,".
" AVG(Rating) AS Avg_Rating FROM PROPERTY LEFT JOIN VISIT ON ID=P_ID WHERE Admin_Name IS NOT NULL".
...
if ($action == 'search') {
  ...
  if ( $keyword!=""){
    ...
    $newsql = "SELECT Name, ID, Size_Acres, St_Address, City, ZIP, IsPublic, IsComm,Property_Type, Admin_Name,".
" AVG(Rating) AS Avg_Rating FROM PROPERTY LEFT JOIN VISIT ON ID=P_ID WHERE Admin_Name IS NOT NULL AND $column2 LIKE '%$keyword%'".
" GROUP BY ID ORDER BY $column1 $order;";
  }
}
...
if ($action == 'search2') {
  if ($_POST['minsearch']=="" && $_POST['maxsearch']=="") {
    ...
    $newsql = "SELECT Name, ID, Size_Acres,St_Address,City,ZIP,IsPublic, IsComm, Property_Type,Admin_Name, ".
    "AVG(Rating) AS Avg_Rating FROM PROPERTY LEFT JOIN VISIT ON ID=P_ID WHERE Admin_Name IS NOT NULL GROUP BY ID HAVING ";
    if ($_POST['minsearch']!="") {
        ...
        <form class="example" method="post">
          ...
          <div class="single-defination">
                ...
                <select class="genric-btn default height radius" name = "sort_type">
                  ...
                  <option  value="Admin_Name">Verified by</option>
          ...
          <div class="single-defination">
            <div>
              <a href="11_view_cof_pro.php" class="genric-btn primary radius" float='left'>↻</a>
            </div>
          </div>
          </div>
          <br><br><br><br>
...
<h3 class="mb-30 text-center">All Confirmed Properties</h3>
...
<div class="progress-table-wrap">
  <div class="progress-table">
    <div class="table-head">
      ...
      <div class="serial">Verified by</div>
...
if ($result = mysqli_query($link, $sql)) {
  while ($property_data = mysqli_fetch_row($result)) {
    ...
    <div class="table-row">
      ...
      <div class="serial"><?php echo $adminname ?></div>
